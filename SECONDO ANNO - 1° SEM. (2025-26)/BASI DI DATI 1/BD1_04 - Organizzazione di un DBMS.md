...
Per **accedere concretamente a un determinato bucket** di un random file, si mantiene in memoria principale una cosiddetta "**bucket directory**", ossia un insieme di entità dove ognuna corrisponde a un puntatore al primo blocco del bucket associato ad essa.
...
Per comprendere meglio come lavorare con questo tipo di file, vediamo un caso concreto. Supponiamo di avere un file con un numero $NR$ di entità pari a $1\,857\,000$, dove ogni entità occupa uno spazio $RS=256\text{ byte}$; supponiamo anche di lavorare con blocchi di memoria di dimensione $BS=4096\text{ byte}$, sapendo che un puntatore a un blocco occupa $PS=5\text{ byte}$. Si vuole organizzare questo file con una struttura random file, con un numero di bucket pari a $NBK=300$, e si vuole sapere il numero di blocchi di memoria necessari per fare ciò, così come il numero medio di accessi necessari per trovare un'entità specifica all'interno di tale file. Partiamo, innanzitutto, quantificando le caratteristiche della bucket directory: come abbiamo detto, essa è costituita sostanzialmente da una lista di puntatori a bucket, dunque è questo tipo di dato ad interessarci ora. Per sapere quanti blocchi sono necessari per contenere la bucket directory, vogliamo trovare innanzitutto la quantità di puntatori a bucket contenibili in un blocco di memoria della stessa:
$$PpB=\frac{BS}{PS}=\frac{4096}{5}=819\text{ puntatori}$$
Dunque, sapendo che un blocco di memoria può contenere $819$ puntatori, vediamo quanti blocchi sono necessari per la bucket directory dividendo il numero di bucket per la quantità appena trovata:
$$NBpBD=\frac{NBK}{PpB}=\frac{300}{819}\approx 1\text{ blocco}$$
Passiamo ora ai bucket effettivi. La quantità di entità contenibili in un singolo blocco di memoria è dato da:
$$RpB=\frac{BS-PS}{RS}=\frac{4096-5}{256}\approx 15\text{ entita'}$$
mentre il numero di entità che dovrà essere inserito in ciascun bucket è:
$$RpBK=\frac{NR}{NBK}=\frac{1\,857\,000}{300}=6190\text{ entita'}$$
Di conseguenza, il numero di blocchi di memoria necessari per ogni bucket sarà pari al rapporto tra quest'ultima quantità e il numero di entità contenibili in un singolo blocco:
$$NBpBK=\frac{RpBK}{RpB}=\frac{6190}{15}=413\text{ blocchi}$$
Ora, possiamo trovare il numero totale di blocchi necessari per contenere i dati desiderati; tale numero sarà dato dalla somma dei blocchi necessari per la bucket directory e il numero di blocchi che servono per tutti i bucket effettivi:
$$NB_{tot}=(NBK\cdot NBpBK)+NBpBD=300\cdot 413+1=123\,901\text{ blocchi}$$
Come accennato in precedenza, per accedere a un'entità ben precisa contenuta in un random file sarà necessario $1$ RBA per entrare nel bucket corretto e un certo numero di SBA per arrivare al blocco desiderato: in questo esempio, il numero medio di questi ultimi accessi sarà $\frac{413}{2}\approx 207$. 
___
##### Indexed sequential file
...
Un indexed sequential file è costituito da un **file principale**, dove sono contenute le entità concrete, e da un **file indice**, utilizzato per accedere ai dati del primo. Il file principale viene diviso in **intervalli**, detti anche "**partizioni**", e ciascuna partizione è indicata da un **indice** contenente la **chiave di ricerca della prima entità della partizione** e un **puntatore alla posizione fisica di tale entità**. In particolare, quest'ultimo può essere:
...
Concretamente, il file principale è un sequential file, in cui le entità sono ordinate in base al valore delle loro chiavi di ricerca; anche il file degli indici è ordinato allo stesso modo.
...
Dato un numero $NB_{FP}$ di blocchi occupati dal file principale, e un numero $NB_{FI}$ di blocchi occupati dal file indice, possiamo quantificare l'**efficienza della ricerca di un'entità** in base a vari approcci possibili:
- se volessimo utilizzare una **ricerca lineare esclusivamente sul file principale**, allora si potrebbe arrivare anche a effettuare $NB_{FP}$ accessi di tipo SBA;
- se volessimo utilizzare una **ricerca binaria esclusivamente sul file principale**, allora ci si aspettano $\log_{2}(NB_{FP})$ accessi di tipo RBA;
- se volessimo utilizzare una **ricerca binaria sul file indice**, allora ci si aspettano $\log_{2}(NB_{FI})$ accessi di tipo RBA.

Va da sé che l'ultimo dei tre approcci risulta più efficiente sia, banalmente, del primo che del secondo, dato che per natura del file indice esso sarà composto da meno blocchi di memoria del file principale, ed effettuare una ricerca su di esso richiederà dunque meno accessi.

Per comprendere meglio come lavorare con questo tipo di file, vediamo un caso concreto. Supponiamo di avere un file con un numero $NR$ di entità pari a $1\,750\,000$, dove ogni entità occupa uno spazio $RS=130\text{ byte}$, di cui $KS=35\text{ byte}$ per la chiave di ricerca; supponiamo anche di lavorare con blocchi di memoria di dimensione $BS=2048\text{ byte}$, sapendo che un puntatore a un blocco occupa $PS=5\text{ byte}$. Si vuole organizzare questo file con una struttura ISAM (ossia con un indexed sequential file), e si vuole sapere il numero di blocchi di memoria necessari per fare ciò. Partiamo quantificando i blocchi necessari per contenere il file principale, e troviamo innanzitutto la quantità di entità contenibile in un singolo blocco di memoria:
$$RpB=\frac{BS}{RS}=\frac{2048}{130}\approx 15\text{ entita'}$$
Dunque, per trovare il numero di blocchi necessari per il file principale, basterà dividere il numero di entità per quante entità possiamo avere in un singolo blocco:
$$NBpFP=\frac{NR}{RpB}=\frac{1\,750\,000}{15}=116\,667\text{ blocchi}$$
Passiamo, ora, al file indice. Abbiamo specificato in precedenza che ogni entità del file indice è composta da una chiave di ricerca e un puntatore, dunque la dimensione di ciascuna entità del file indice sarà:
$$ES=KS+PS=35+5=40\text{ byte}$$
Vediamo quante di queste entità particolari possono essere contenute in un blocco:
$$EpB=\frac{BS}{ES}=\frac{2048}{40}\approx 51\text{ entita'}$$
Di conseguenza, dato che generalmente si considera che ogni entità del file indice punta a un singolo blocco del file principale, il numero di blocchi necessari per il file indice sarà pari al rapporto tra i blocchi del file principale e il numero di entità del file indice contenute in un singolo blocco, ossia:
$$NBpFI=\frac{NBpFP}{EpB}=\frac{116\,667}{51}\approx 2288\text{ blocchi}$$
Abbiamo così che il numero complessivo di blocchi di memoria necessari per questa organizzazione ISAM sarà $NBpFP+NBpFI=116\,667+2288=118\,955\text{ blocchi}$.
___
## Alberi
